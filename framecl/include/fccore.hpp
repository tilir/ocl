...
public:
  ...
  void select(int n) {
    assert(n >= 0 && n < size() && "can not select device: number incorrect, check size()");
    active_ = n;
  }
...
struct ITask {
  virtual void execute(cl::Context c, cl::CommandQueue q, eventguards_t &evts) = 0;
  virtual ~ITask() {}
};
...
public:
  ...
  task_t(task type, T& obj, Args&& ... args) : type_{type}, tsk_{ obj.get_task(type, std::forward<Args>(args)...) } {}  
  
  void enqueue_on(cl::Context c, cl::CommandQueue q, eventguards_t &evts) const {
  ...
  void dump(std::ostream& os) const {
    os << "[task: " << tsk_.get() << " ";
    switch(type_) {
      case task::write: os << "write"; break;
      case task::process: os << "process"; break;
...
class functor_t final {
  context_t &ctx_;
  run_params_t rp_;
  cl::KernelFunctor<Ts...> kf_;

public:
  functor_t(program_t prog, run_params_t rp, std::string kname) : ctx_(prog.context()), rp_(rp), kf_(prog, kname) {}
...
public:
  buffer_t(context_t& ctx, int size) : ctx_(ctx), size_(size), contents_(size_), 
...
struct BufferReadTask : public ITask {
  ...
  BufferReadTask(cl::Buffer buf, int off, int sz, void *ptr) : buf_(buf), off_(off), size_(sz), ptr_(ptr) {}
  
  void execute(cl::Context c, cl::CommandQueue q, eventguards_t &evts) override {
    q.enqueueReadBuffer(buf_, /* bloking */ true, off_, size_, ptr_, &evts.ins, &evts.out);
  }
};
...
struct BufferWriteTask : public ITask {
  ...
  BufferWriteTask(cl::Buffer buf, int off, int sz, void *ptr) : buf_(buf), off_(off), size_(sz), ptr_(ptr) {}

  void execute(cl::Context c, cl::CommandQueue q, eventguards_t &evts) override {
    q.enqueueWriteBuffer(buf_, /* bloking */ true, off_, size_, ptr_, &evts.ins, &evts.out);
  }
};

template<typename F, typename ... Args>
struct ProgramExecTask : public ITask {  
  run_params_t rp_;
  F func_;
  std::function<cl::Event(cl::EnqueueArgs)> f_;  
  
  // not perfect forwarding because lambda will be used out of scope, so I don't want to mess with dangling stuff
  // TODO: rethink it later
  ProgramExecTask(run_params_t rp, F func, Args ... args) : rp_(rp), func_(func) {
...
template <typename T>
std::unique_ptr<ITask> buffer_t<T>::get_task(task type) {
  if (type == task::read)
    return std::unique_ptr<ITask>{ new BufferReadTask(buf_, 0, size_ * sizeof(T), contents_.data()) };
  if (type == task::write)
    return std::unique_ptr<ITask>{ new BufferWriteTask(buf_, 0, size_ * sizeof(T), contents_.data()) };
...
std::unique_ptr<ITask> functor_t<Ts...>::get_task(task type, Vs&& ... args) {
  if (type == task::process)
    return std::unique_ptr<ITask>{ new ProgramExecTask(rp_, kf_, std::forward<Vs>(args)...) };
...
public:
  depgraph_t(context_t& ctx, std::initializer_list<std::initializer_list<task_t*>> init) : ctx_(ctx) {
    ...
    for(;;) {
      ...
      for(auto&& pt : tasks_) {
        ...
        if (level != -1) {
          ...
          assert(level + 1 <= level_tasks_.size() && "This invariant shall hold by definition");
    ...
    for (auto&& lv : level_tasks_) {
      ...
      for (auto&& pt : lv) {
        ...
        for (auto &&pdep : deps_[pt]) {
          ...
          int status = evts_[dep_id].getInfo<CL_EVENT_COMMAND_EXECUTION_STATUS>();          
