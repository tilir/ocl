...
public:
  optparser_t() {
    ...
    desc_.add_options()("list-devices", po::value<std::string>(),
                        "List available devices for given platform (matches substr in name)");
    desc_.add_options()("info", po::value<std::string>(), 
    ...
    desc_.add_options()("device", po::value<std::string>(), 
  ...
  void add(std::string name, T defval, std::string description = "") {
    ...
    desc_.add_options()(name.c_str(), po::value<T>()->default_value(defval), description.c_str());
  }
  ...
  void parse(int argc, char **argv) {
    ...
    if (!quiet_) {
      std::cout << "Info: targeting OCL_VERSION = " << OCL_VERSION << " rebuild to change" << std::endl;
  ...
  int check(std::string name, F checker, std::string emsg = "") const {
    assert(parsed_ && "Please do not invoke checkers before options are parsed");
    T val = vm_[name].as<T>();
    if (!checker(val)) {
  ...
  void require(std::string what, std::string adderr) const {
    assert(parsed_ && "Please do not check options for existence before they are parsed");
    if (0 == vm_.count(what)) {
      std::ostringstream os;
  ...
  void require_program(std::string adderr) const {
    require("program", adderr);
  }
