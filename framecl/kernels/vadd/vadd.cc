...
int vaddmain(int argc, char **argv) {
  ...
  int size =  opts.check<int>("size", [](int sz) { return sz > 0; }, "size shall be > 0");
  ...
  opts.require_platform("This program requires platform specification. Use --list for available platforms");
  opts.require_program("This program needs external cl program file. It shall contain 'vadd' kernel");
  ...
  framecl::functor_t<cl::Buffer, cl::Buffer, cl::Buffer> vadd(prog, parms, "vector_add");
  ...
  if (opts.verbose()) {
    std::cout << "bufA: "; bufA.dump(std::cout); std::cout << std::endl;
    std::cout << "bufB: "; bufB.dump(std::cout); std::cout << std::endl;
  }
  ...
  framecl::task_t execF(framecl::task::process, vadd, bufA.base(), bufB.base(), bufC.base());
  framecl::task_t readC(framecl::task::read, bufC);

  // dependency graph to execute as a graph (like DPC++ but in framework, not in language)
  ...
  if (opts.verbose()) {
    std::cout << "bufC: "; bufC.dump(std::cout); std::cout << std::endl;
  }  
