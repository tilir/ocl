...
static sycl::event
EnqueueFilterSampler(sycl::queue &DeviceQueue, sycl::image<2> Dst,
                     sycl::image<2> Src, sycl::range<2> Dims,
                     sycl::buffer<sycl::float4, 1> FiltBuffer, int HalfWidth) {
...
public:
  ...
  sycltesters::EvtRet_t operator()(sycl::float4 *DstData, sycl::float4 *SrcData,
    ...
    auto Evt = EnqueueFilterSampler(DeviceQueue, Dst, Src, Dims, FiltBuffer,
                                    HalfWidth);
