...
# see reduction.plot on how to collect data
#
#------------------------------------------------------------------------------
#
...
puts "Running Reductions";

ARGV << '-h' if ARGV.empty?

options = OpenStruct.new
options.verbose = false
options.progname = "reduction\\reduce_naive.exe"
options.outfile = "reduce.dat"
options.lsz = 256
options.bsz = 1024
...
def run_reduce(progname, outfile, szx, sz0, szmult, lsz, bsz)
  sz = szx * szmult
...
(sz0..szn).each do |sz|
  run_reduce(options.progname, options.outfile, sz, sz0, options.szmult, options.lsz, options.bsz)
end
